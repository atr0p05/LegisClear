...
import { FileText, Search, RefreshCw, Settings, Filter } from 'lucide-react';
...
interface ActiveContextManagerProps {
  isOpen: boolean;
  onActiveDocumentsChange: (activeDocumentIds: string[]) => void;
  className?: string;
}

export const ActiveContextManager: React.FC<ActiveContextManagerProps> = ({
  isOpen,
  ...
  const getTypeColor = (type: Document['type']) => {
    const colors = {
      ...
      treatise: 'bg-indigo-100 text-indigo-800',
          ...
          <div className="bg-blue-50 p-3 rounded-lg">
            ...
            <div className="space-y-1">
              {activeDocuments.slice(0, 3).map(doc => (
                <div key={doc.id} className="text-xs text-blue-700 truncate">
                  {doc.name}
                </div>
              ))}
              ...
              filteredDocuments.map((document) => (
                <div
                  ...
                  className={`p-3 border rounded-lg transition-colors ${
                    document.isActive ? 'bg-blue-50 border-blue-200' : 'hover:bg-slate-50'
                  }`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-500 mb-2">
                        ...
                        <span>{document.size.toFixed(1)} MB</span>
